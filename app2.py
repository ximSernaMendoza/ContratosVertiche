...
/* Código Civil Federal (Doc obligatorio) */

/* Fondo del input deshabilitado */
input[disabled] {{
...
/* Botones Modo Consulta*/
/* RADIO: círculo exterior */
div[role="radiogroup"] label {{
    color: #966368 !important;
}}

/* círculo exterior normal */
div[role="radiogroup"] label > div:first-child {{
    border: 2px solid #966368 !important;
    background: #966368 !important;
...
if section == "consulta":
    ...
    for (label, prompt), col in zip(chip_prompts.items(), cols):
        with col:
            if st.button(label, key=f"chip_{label}", use_container_width=True):
                ...
                st.session_state.auto_submit_question = prompt
                st.rerun()
    ...
    if st.session_state.auto_submit_question:
        submitted_question = st.session_state.auto_submit_question
