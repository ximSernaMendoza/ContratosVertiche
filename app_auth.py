...
def _log_access(archivo: str, accion: str):
    try:
...
SUPABASE_KEY = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Imx2dGhjaGdhc3BmYnV5YnRya29lIiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzE2NjU1NDksImV4cCI6MjA4NzI0MTU0OX0.lhgBxzFlmyT7Nj9CxB_puYTiVJiwKNsDt6jNrnG-o-I"
...
def upload_to_bucket(file_bytes: bytes, dest_path: str, content_type: Optional[str] = None):
    ...
    result = bucket.upload(dest_path, file_bytes, file_options=opts)
    _log_access(dest_path, "upload")
    return result
...
def _session_expired() -> bool:
    """Verifica si el JWT del usuario ya expiró leyendo el campo 'exp'."""
...
if st.session_state.get("user") is None or _session_expired():
    if _session_expired() and st.session_state.get("user"):
...
# Restaurar sesión con el token del usuario para que RLS funcione
supabase.auth.set_session(
    st.session_state["access_token"],
    st.session_state["refresh_token"] or "",
)
# Inicializar bucket con la sesión autenticada del usuario
bucket = supabase.storage.from_(BUCKET_NAME)

# Cargar rol del usuario (admin u otro)
if "is_admin" not in st.session_state:
    try:
        user_id = st.session_state["user"].id
        role_resp = supabase.table("user_roles").select("role").eq("user_id", str(user_id)).execute()
        roles = [r["role"] for r in (role_resp.data or [])]
...
with st.sidebar:
    ...
    if st.session_state.get("is_admin"):
        st.markdown("---")
        if st.button("Admin: Logs", key="admin_link"):
...
if section == "consulta":
    ...
    st.markdown("### Panel de Administración — Logs de acceso")

    col_f1, col_f2, col_f3 = st.columns([1, 1, 1])
    with col_f1:
        filtro_accion = st.selectbox("Acción", ["Todas", "read", "upload"], index=0)
    with col_f2:
        filtro_fecha = st.date_input("Desde", value=None, key="log_fecha")
    with col_f3:
        if st.button("Actualizar", key="refresh_logs"):
            pass  # Streamlit re-ejecuta automáticamente

    try:
        query = supabase.table("access_log").select("created_at, accion, archivo, user_id").order("created_at", desc=True).limit(200)
        if filtro_accion != "Todas":
            query = query.eq("accion", filtro_accion)
        if filtro_fecha:
            query = query.gte("created_at", filtro_fecha.isoformat())
        logs = query.execute().data or []

        if logs:
            df_logs = pd.DataFrame(logs)
            df_logs["created_at"] = pd.to_datetime(df_logs["created_at"]).dt.strftime("%Y-%m-%d %H:%M:%S")
            df_logs.columns = ["Fecha", "Acción", "Archivo", "Usuario (ID)"]
            st.dataframe(df_logs, use_container_width=True, hide_index=True)
            st.caption(f"{len(logs)} registros mostrados")
        else:
